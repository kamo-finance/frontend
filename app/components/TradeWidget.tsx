	...
	const onChangeInput = async (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		try {
			...
			if (fromToken.type === "SY" && toToken.type === "PT") {
				...
				const toAmount = await market.swapExactSyForYo({
					syAmount: BigInt(Math.floor(fromAmount * 10 ** 6)),
					syExchangeRate: exchangeRate,
					now: Date.now(),
				});

				setToAmount((Number(toAmount) / 10 ** 6).toString());
			} else if (fromToken.type === "YT" && toToken.type === "SY") {
				const toAmount = market.swapExactYoForSy({
					yoAmount: BigInt(Math.floor(fromAmount * 10 ** 6)),
					syExchangeRate: exchangeRate,
					now: Date.now(),
				});
	...
	const handleTrade = async () => {
		...
		if (fromToken.type === "SY" && toToken.type === "PT") {
			...
			await kamoTx.swapYoForSy({
				yoAmount: BigInt(Math.floor(parseFloat(fromAmount) * 10 ** 6)),
				sender: account.address,
				tx: tx,
			});
		}
